...
if(Array.isArray(JSON.parse(localStorage.getItem('rooms'))) ) {
    let savedRooms = JSON.parse(localStorage.getItem('rooms'));
    if(room!=="" && !savedRooms.includes(room)) {
        localStorage.setItem('rooms', JSON.stringify([...savedRooms, room]));
    }
} else {
...
function notif(messages) {
    if(!Notification.permission) {
        ...
        let msg =  messages[messages.length == 1?0:messages.length-1];
        console.log(msg);

        const notif = new Notification(msg.title+' sur '+msg.room, {tag: 'message'+msg.id, body: msg.content});
    }
}
